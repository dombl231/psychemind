import { buildEbookPackagePrompt, callOpenAI, ebookPackageSchema, enforceRateLimit, getOutputText, HttpError, json, normalizeGenerateInput, readJson, requireSameOrigin, verifyTurnstile } from "../_lib/ebook-api.js";

export async function onRequestPost({ request, env }) {
  try {
    requireSameOrigin(request);
    enforceRateLimit(request, { keyPrefix: "generate-package", limit: Number(env.EBOOK_RATE_LIMIT || 4), windowMs: 60 * 60 * 1000 });
    ...
    return json({ error: error.message || "전자책 기획 생성에 실패했습니다." }, error instanceof HttpError ? error.status : 500);
  }
}
