...
- Fix applied: `requireSameOrigin(request)` now rejects browser cross-origin calls; generation input is size-limited and clamped; `max_output_tokens` limits response size.
- Residual risk: server-to-server clients can still call public endpoints. Before paid launch, add Cloudflare WAF rate limiting, Turnstile, payment entitlement checks, or account/session gating.
...
- Add Turnstile or payment entitlement checks before public paid launch.
